...
impl RedisNewUser {
    ...
    pub async fn insert(&self, redis: &RedisPool, ulid: &Ulid) -> Result<(), ApiError> {
        ...
        redis.hset::<(),_,_>(&key_email, hmap!(ulid.to_string())).await?;
        redis.expire::<(),_>(key_email, Self::TTL_AS_SEC.into()).await?;
        redis.hset::<(),_,_>(&key_secret, hmap!(new_user_as_string)).await?;
