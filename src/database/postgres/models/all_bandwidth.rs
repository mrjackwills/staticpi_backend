...
SELECT
    ...
    ( SELECT COALESCE(SUM(size_in_bytes), 0)::BIGINT FROM hourly_bandwidth WHERE timestamp >= NOW() - INTERVAL '24 hours' AND timestamp <= NOW() AND is_counted = FALSE) as day_in,
    ( SELECT COALESCE(SUM(size_in_bytes), 0)::BIGINT FROM hourly_bandwidth WHERE timestamp >= NOW() - INTERVAL '24 hours' AND timestamp <= NOW() AND is_counted = TRUE) as day_out,
    ( SELECT COALESCE(SUM(size_in_bytes), 0)::BIGINT FROM hourly_bandwidth WHERE extract(year from timestamp) = extract (year FROM CURRENT_DATE) AND extract(month FROM timestamp) = extract (month FROM CURRENT_DATE) AND is_counted = FALSE) as month_in,
