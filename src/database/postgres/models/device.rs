...
impl ModelApiKey {
    ...
    async fn create_api_key(
        ...
        loop {
            ...
            if Self::get(transaction, &api_key).await?.is_none() {
                break Ok(api_key);
            }
        }
    }
...
WHERE
    ...
    async fn insert(
        ...
        if let Some(device_name) = sqlx::query_as!(
            Self,
            "SELECT
    device_name_id,
    name_of_device
...
FROM
    ...
    JOIN api_key ap USING(api_key_id)
    JOIN device_name dn USING(device_name_id)
    JOIN user_level ul ON ul.user_level_id = ru.user_level_id
WHERE
    de.active = TRUE
    AND de.paused = FALSE
    AND de.device_id = $1";
...
WHERE
    device_id = $2",
            device_name.device_name_id.get(),
            self.device_id.get()
        )
        .execute(&mut *transaction)
        .await?;
...
VALUES
    ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
            ...
            device_name.device_name_id.get(),
            api_key_id.get(),
            new_device.max_clients,
            client_password_id.map(DevicePasswordId::get),
            device_password_id.map(DevicePasswordId::get),
            new_device.structured_data
        )
        .execute(&mut *transaction)
        .await?;
        transaction.commit().await?;
