...
impl AdminModelUser {
    ...
    pub async fn get_all(
        ...
        let query = r"
SELECT
    ru.active, ru.full_name, ru.timestamp::TEXT, ru.registered_user_id,
    ea.email,
    COALESCE(la.login_attempt_number, 0) AS login_attempt_number,
    CASE WHEN tfs.two_fa_secret IS NULL THEN FALSE ELSE TRUE END as two_fa_enabled,
    CASE WHEN (
        SELECT pr.password_reset_id FROM password_reset pr WHERE pr.registered_user_id = ru.registered_user_id AND pr.timestamp >= NOW () - INTERVAL '1 hour' AND pr.consumed IS NOT TRUE
    ) IS NULL THEN FALSE ELSE TRUE END AS password_reset,
    (
        SELECT
        COALESCE(COUNT(*),0)
        FROM
