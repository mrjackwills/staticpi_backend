...
impl ModelEmailLog {
    ...
    pub async fn get_count_hour(postgres: &PgPool) -> Result<Count, ApiError> {
        let query ="SELECT COUNT(*) FROM email_log WHERE sent = TRUE AND timestamp >= NOW() - INTERVAL '1 hour'";
    ...
    pub async fn get_count_total(postgres: &PgPool) -> Result<Count, ApiError> {
        let query = "SELECT COUNT(*) FROM email_log";
        Ok(sqlx::query_as::<_, Count>(query)
            .fetch_one(postgres)
            .await?)
    }
    ...
    pub async fn update_sent_false(&self, postgres: &PgPool) {
        let query = "UPDATE email_log SET sent = FALSE where email_log_id = $1";
    ...
    pub async fn insert(
        ...
        let query = "INSERT INTO email_log (ip_id, user_agent_id, email_address_id, email_subject_id) VALUES ($1, $2, $3, $4) RETURNING email_log_id";
