...
GRANT USAGE, SELECT ON SEQUENCE ip_address_ip_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE user_agent_user_agent_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE user_level_user_level_id_seq TO staticpi;
CREATE INDEX user_level_index ON user_level(user_level_name);


-- Insert different user levels into db
INSERT INTO user_level(custom_device_name, max_number_of_devices, max_message_size_in_bytes, max_monthly_bandwidth_in_bytes, max_clients_per_device, structured_data, device_password, user_level_name) VALUES(false, 1, 10 * 1000, 5 * 1000000, 1,  FALSE, FALSE, 'free');
INSERT INTO user_level(custom_device_name, max_number_of_devices, max_message_size_in_bytes, max_monthly_bandwidth_in_bytes, max_clients_per_device, structured_data, device_password, user_level_name) VALUES(true, 20, 5 * 1000 * 1000, 100 * 100000000::bigint, 100,  TRUE, TRUE, 'pro');
INSERT INTO user_level(custom_device_name, max_number_of_devices, max_message_size_in_bytes, max_monthly_bandwidth_in_bytes, max_clients_per_device, structured_data, device_password, user_level_name) VALUES(true, 20, 10 * 1000 * 1000, 100 * 1000000000::bigint, 100,  TRUE, TRUE, 'admin');
...
GRANT USAGE, SELECT ON SEQUENCE email_address_email_address_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE device_password_device_password_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE registered_user_registered_user_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE invite_code_invite_code_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE password_reset_password_reset_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE login_history_login_history_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE login_attempt_login_attempt_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE two_fa_secret_two_fa_secret_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE two_fa_backup_two_fa_backup_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE api_key_api_key_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE device_name_device_name_id_seq TO staticpi;
...
CREATE TABLE IF NOT EXISTS device (
	...
	max_clients SMALLINT DEFAULT 1 CHECK(max_clients > 0 AND max_clients < 101),
	active BOOLEAN DEFAULT true
);

GRANT ALL ON device TO staticpi;
GRANT USAGE, SELECT ON SEQUENCE device_device_id_seq TO staticpi;
-- Unique index, so that a single user cannot have two devices that are both active with the same name
CREATE UNIQUE INDEX unique_user_device_name ON device (device_name_id, registered_user_id) WHERE (active is true);
...
GRANT USAGE, SELECT ON SEQUENCE connection_connection_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE email_subject_email_subject_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE email_log_email_log_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE contact_message_contact_message_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE banned_email_domain_banned_email_domain_id_seq TO staticpi;
...
GRANT USAGE, SELECT ON SEQUENCE hourly_bandwidth_hourly_bandwidth_id_seq TO staticpi;
